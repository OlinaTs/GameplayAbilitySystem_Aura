...
struct AuraDamageStatics
{
	...
	AuraDamageStatics()
	{
		...
		// the attacker is what uses ArmorPenetration
...
void UExecCalc_Damage::DetermineDebuffs(const FGameplayEffectCustomExecutionParameters& ExecutionParams, const FGameplayEffectSpec& Spec, FAggregatorEvaluateParameters EvaluationParameters, const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (auto Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		// this float will be -1 for any DamageTypes that don't exist as a SetByCallerMagnitude in our Spec.
		const float TypeDamage = Spec.GetSetByCallerMagnitude(DamageType, false, -1.f);

		// if (TypeDamage > -1) that means we have the DamageType that we're interested in 
		if (TypeDamage > -1)
		{
			// Determine if this was a successful Debuff
			const float SourceDebuffChance = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Chance, false, -1.f);

			float TargetDebuffResistance = 0.f;
			const FGameplayTag& ResistanceTag = GameplayTags.DamageTypesToResistances[DamageType];
			// Look up the target’s resistance attribute that matches this damage type,
			// calculate its effective value with all modifiers, and store it in TargetDebuffResistance
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// Debuff
	DetermineDebuffs(ExecutionParams, Spec, EvaluationParameters, TagsToCaptureDefs);
	...
	// we make sure TargetBlockChance is not a negative value
	...
	// this will be the amount of Armor leftover after we ignored a percentage of it
	...
	// From Critical Hit Chance, we subtract a percentage of Critical Hit Resistance
	const float EffectiveCriticalHitChance = SourceCriticalHitChance - TargetCriticalHitResistance * CriticalHitResistanceCoefficient;
	const bool bCriticalHit = FMath::RandRange(1, 100) < EffectiveCriticalHitChance;

	UAuraAbilitySystemLibrary::SetIsCriticalHit(EffectContextHandle, bCriticalHit);
	
	// if Critical hit, then double damage plus a bonus
