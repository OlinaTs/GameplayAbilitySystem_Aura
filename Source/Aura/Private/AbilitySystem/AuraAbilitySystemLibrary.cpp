	...
	FWidgetControllerParams& OutWCParams, AAuraHUD*& OutAuraHUD)
{
	if (APlayerController* PC = UGameplayStatics::GetPlayerController(WorldContextObject, 0))
	{
		...
		if (OutAuraHUD)
		{
			...
			// We can get the AbilitySystemComponent and the AttributeSet from the Player State
...
void UAuraAbilitySystemLibrary::InitializeDefaultAttributesFromSaveData(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ULoadScreenSaveGame* SaveGame)
{
	...
	EffectContexHandle.AddSourceObject(SourceAvatarActor);
	
	const FGameplayEffectSpecHandle SpecHandle = ASC->MakeOutgoingSpec(CharacterClassInfo->PrimaryAttributes_SetByCaller, 1.f, EffectContexHandle);
	
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Get().Attributes_Primary_Strength, SaveGame->Strength);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Get().Attributes_Primary_Intelligence, SaveGame->Intelligence);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Get().Attributes_Primary_Resilience, SaveGame->Resilience);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Get().Attributes_Primary_Vigor, SaveGame->Vigor);
...
void UAuraAbilitySystemLibrary::GetLivePlayersWithinRadius(const UObject* WorldContextObject, TArray<AActor*>& OutOverlappingActors, const TArray<AActor*>& ActorsToIgnore, float Radius, const FVector& SphereOrigin)
{
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			// we check to see if this Actor...
			// If the Actor Implements the CombatInterface and IsAlive
			if (Overlap.GetActor()->Implements<UCombatInterface>() && !ICombatInterface::Execute_IsDead(Overlap.GetActor()))
			{
				// And if it does, we add the Actor to the OutOverlappingActors TArray
...
void UAuraAbilitySystemLibrary::GetClosestTargets(int32 MaxTargets, const TArray<AActor*>& Actors, TArray<AActor*>& OutClosestTargets, const FVector& Origin)
{
	...
	// this is a non-const reference, so we can make calculations with it
	TArray<AActor*> ActorsToCheck = Actors;
	// so we can keep track as soon as we find an Actor that we know is one
	// of the closest ones and increment this integer
	...
	while (NumTargetsFound < MaxTargets)
	{
		...
		// because we've already checked and saw it was the Closest Actor
...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(const FDamageEffectParams& DamageEffectParams)
{
	...
	FGameplayEffectContextHandle EffectContexHandle = DamageEffectParams.SourceAbilitySystemComponent->MakeEffectContext();
	EffectContexHandle.AddSourceObject(SourceAvatarActor);
	SetDeathImpulse(EffectContexHandle, DamageEffectParams.DeathImpulse);
	SetKnockbackForce(EffectContexHandle, DamageEffectParams.KnockbackForce);

	SetIsRadialDamage(EffectContexHandle, DamageEffectParams.bIsRadialDamage);
	SetRadialDamageInnerRadius(EffectContexHandle, DamageEffectParams.RadialDamageInnerRadius);
	SetRadialDamageOuterRadius(EffectContexHandle, DamageEffectParams.RadialDamageOuterRadius);
	SetRadialDamageOrigin(EffectContexHandle, DamageEffectParams.RadialDamageOrigin);
	
	const FGameplayEffectSpecHandle SpecHandle = DamageEffectParams.SourceAbilitySystemComponent->MakeOutgoingSpec(DamageEffectParams.DamageGameplayEffectClass, DamageEffectParams.AbilityLevel, EffectContexHandle);
